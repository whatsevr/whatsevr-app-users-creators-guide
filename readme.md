# Whatsevr App User Documentation

Welcome to the official user documentation for the **Whatsevr App**. This guide will help you navigate the application, manage your account, and make the most of its features.

## Table of Contents

1. [Getting Started](01-getting-started.md)
   - Installation & App Launch
   - Creating an Account (New Users)
   - Logging In (Existing Users)
   - Account Switching
   - Special Account States (Banned/Deactivated)
   - First-Time User Experience
   - Profile Setup Basics
   - Troubleshooting
2. [Discovering Content](04-discovering-content.md)
   - Home Feed & Personalized Recommendations
   - For You Tab (Personalized Feed)
   - Communities, Offers & Activities Tabs
   - Activity History & Saved Collections
   - Explore Page & Content Tabs
   - Flicks Feed (Short-Form Videos)
   - Search Functionality Across All Content Types
   - Search Filters & Sorting Options
   - Navigating Between Content Views
   - Tips for Effective Discovery
   - Troubleshooting
3. [Engaging with Content](05-engaging-with-content.md)
   - Commenting on Posts (with replies, images, tagging)
   - Reactions (6 types: like, love, haha, wow, sad, angry)
   - Viewing Who Reacted & Filtering
   - Sharing Content (to users and communities)
   - Following and Unfollowing Users
   - Viewing Post Details
   - Best Practices for Community Engagement
   - Engagement Notifications
   - Troubleshooting
4. [Communities](06-communities.md)
   - What Communities Are & Their Purpose
   - Discovering Top Communities
   - Creating a New Community
   - Joining and Leaving Communities
   - Viewing Community Details & Content
   - Community Tabs (About, Services, Media, Videos, Offers, Tags)
   - Community Member Management (For Owners)
   - Posting Content to Communities
   - Updating Community Profile & Cover Media
   - Community-Specific Features & Settings
   - Best Practices & Community Guidelines
   - Troubleshooting
5. [Messaging & Chat](07-messaging-and-chat.md)
   - Accessing the Messages Tab
   - Understanding the Messages Interface
   - Starting New Conversations
   - Sending Messages (Text, Images, Files)
   - Message Composer Features
   - Sharing Content in Messages
   - Message Actions and Management
   - Understanding Message Bubbles
   - Online Users Feature
   - Community Chats vs Private Chats
   - Conversation Navigation and Features
   - Message Notifications
   - Privacy and Safety Tips
   - Best Practices for Messaging
   - Troubleshooting
6. [Profile & Account Management](08-profile-and-account.md)
   - Viewing Your Own Profile vs Others' Profiles
   - Editing Profile Information
   - Adding Work Experience and Education
   - Portfolio Management
   - Updating Profile Picture and Cover Media
   - Username Changes
   - Account Settings Overview
   - Privacy Settings (Active Login Sessions, Device Permissions)
   - Clear Cache
   - Profile Tabs Overview
   - Profile Statistics
   - Profile Optimization Tips
   - Account Actions (Add/Switch Account, Logout)
   - Troubleshooting
   - Best Practices
7. [Core Features](features.md)
   - Home Feed
   - Flicks
   - Communities
   - Chat & Calls
   - Sneekpeek
   - AI Chat
   - Search & Explore
   - Sharing
8. [Creating Other Content](03-creating-other-content.md)
   - Creating Photo Posts
   - Creating Memories
   - Creating Offers
   - Uploading PDF Documents
   - Content-Specific Fields Comparison
   - Understanding Posting Contexts
   - Image Editing & Cropping Features
   - Technical Requirements & Limits
   - Best Practices
   - Troubleshooting
9. [Content Creation](content-creation.md)
   - Creating Posts
   - Uploading Flicks
   - Types of Content
   - Image Editing
10. [Account Management](account-management.md)
   - Profile Settings
   - Portfolio & Services
   - Privacy & Security
11. [Advanced Features](advanced-features.md)
   - EvrMind AI
   - Creator Studio
   - Collections
   - Reporting
12. [Troubleshooting & Support](support.md)

---

## Overview

Whatsevr is a comprehensive social platform designed to connect you with communities, friends, and content you love. Whether you're here to share your authentic moments through **WTV videos**, enjoy entertaining **Flicks**, share multiple **Photo Posts** with your audience, create temporary **Memories**, post promotional **Offers**, upload professional **PDF Documents**, participate in vibrant **Communities**, access exclusive **Sneekpeek** content, chat with our AI assistant **EvrMind**, or engage in real-time messaging and calls, Whatsevr has something for everyone.

### Key Features

- **WTV Videos**: Long-form video content for sharing your stories
- **Flicks**: Short-form vertical videos for entertainment
- **Photo Posts**: Share multiple images with captions, location tags, and user mentions
- **Memories**: Temporary story-like content (images or videos) that stays visible for 1-3 days
- **Offers**: Promotional content for job postings, collaborations, services, and deals with rich formatting
- **PDF Documents**: Upload professional documents, portfolios, and certificates to your portfolio
- **Communities**: Create or join communities around shared interests, professions, or causes. Public communities allow instant joining, while private communities require owner approval. Community owners can manage members, post content, customize profiles with cover media and services, and moderate their community. Members can view community content across multiple tabs (About, Services, Media, Videos, Offers, Tags), chat with the community, and engage with posts.
- **Content Discovery**: Personalized Home feed with recommendations, Explore page with multiple content tabs (Wtv, Photos, Flicks, Offers, Memories), unified search across all content types with advanced filters, and activity history tracking
- **Social Engagement**: Comprehensive interaction system with 6 reaction types (like, love, haha, wow, sad, angry), nested comments with image attachments and user tagging, content sharing to users and communities with custom messages, and follow/unfollow functionality with persistent state management
- **Sneekpeek**: Exclusive behind-the-scenes content from creators
- **EvrMind AI Chat**: Intelligent chat assistant for conversations
- **Messaging & Chat**: Real-time messaging system with private one-on-one chats and community group chats. Send text messages, share content (posts, profiles, communities), edit and delete messages, view online users, and receive push notifications. Message composer supports multi-line text and attachments (in development). Conversations feature decorative backgrounds, delivery status indicators, and rich content previews for shared items.
- **Profile & Account Management**: Comprehensive profile customization with profile picture and cover media (images/videos with aspect ratio options), personal information editing (name, bio, location, birthday, gender, public contact info), education and work experience tracking with ongoing status support, portfolio mode for professionals with title/status/services/description fields, username updates with AI-powered suggestions, active login session management across devices with remote logout capability, device permissions control with system settings integration, cache management with size display and clearing functionality, and organized profile tabs (About, Services, Media, Videos, Flicks, Offerings, PDFs, Tags) for content organization
- **Multi-Account Support**: Manage multiple accounts seamlessly on one device
- **Streamlined Authentication**: Fast and secure authentication using OTPless service (Phone/Email/WhatsApp)
